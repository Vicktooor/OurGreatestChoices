...
public class EditorObjExporter : ScriptableObject
{
    ...
    private static void MeshesToFile(MeshFilter[] mf, string folder, string filename)
    {
        ...
        Debug.Log(Application.dataPath + "/" + folder + "/" + filename + ".obj");
        using (StreamWriter sw = new StreamWriter(Application.dataPath + "/" + folder + "/" + filename + ".obj"))
        {
            sw.Write("mtllib ./" + filename + ".mtl\n");
	...
	[MenuItem("Custom/Export/Export decal")]
	static void ExportSelectedRoad()
	{
		if (!CreateTargetFolder())
			return;
		...
		{
			EditorUtility.DisplayDialog("Nope, too many object selected", "Select ONE road", "");
			return;
		}
		...
		if (selection[0].GetComponent(typeof(MeshFilter)) != null)
			meshfilter = selection[0].GetComponent(typeof(MeshFilter));
		else
		{
			EditorUtility.DisplayDialog("No mesh", "Selected object doesn't have mesh", "");
			return;
		}

		MeshFilter mf = (MeshFilter)meshfilter;
		MeshesToFile(new MeshFilter[] { mf }, targetFolder + "/Roads", meshfilter.gameObject.name);

		EditorUtility.DisplayDialog("Objects exported", "Exported to " + meshfilter.gameObject.name, "Nice");																											 
	}
